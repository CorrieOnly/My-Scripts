...
local DescriptionFrame = Instance.new("ScrollingFrame")
...
InfoBox.Position = UDim2.new(0.00985116791, 0, 0.0875131339, 0)
...
BottomButtons.Parent = InfoBox
BottomButtons.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
BottomButtons.BackgroundTransparency = 1.000
BottomButtons.Position = UDim2.new(0.0100502511, 0, 0.911764622, 0)
BottomButtons.Size = UDim2.new(0, 975, 0, 35)

UIListLayout.Parent = BottomButtons
...
CopyScriptBloxLink.Parent = BottomButtons
...
UICorner_15.Parent = CopyScriptBloxLink

CopyScriptButton.Name = "CopyScriptButton"
CopyScriptButton.Parent = BottomButtons
...
UICorner_16.Parent = CopyScriptButton

ExecuteButton_3.Name = "ExecuteButton"
ExecuteButton_3.Parent = BottomButtons
...
UICorner_17.Parent = ExecuteButton_3

DescriptionFrame.Name = "DescriptionFrame"
DescriptionFrame.Parent = InfoBox
...
DescriptionFrame.Position = UDim2.new(0.0100000119, 0, 0.406019628, 0)
DescriptionFrame.Size = UDim2.new(0, 975, 0, 253)
DescriptionFrame.CanvasSize = UDim2.new(0, 0, 10, 0)
DescriptionFrame.ScrollBarThickness = 0

InfoBoxDescription.Name = "InfoBoxDescription"
InfoBoxDescription.Parent = DescriptionFrame
...
UICorner_18.CornerRadius = UDim.new(0, 24)
UICorner_18.Parent = PreviewImage
...
UICorner_19.Parent = ProfilePicture
...
createfolders("ScriptBlox/Searcher/ImageCache")

if not isfile("ScriptBlox/Searcher_Cache.json") then
	writefile("ScriptBlox/Searcher_Cache.json", "{}")
end
...
local cache = HttpService:JSONDecode(readfile("ScriptBlox/Searcher_Cache.json"))
...
ScriptBloxSearcher.MainFrame.InfoBox.DescriptionFrame.AutomaticCanvasSize = Enum.AutomaticSize.Y
ScriptBloxSearcher.MainFrame.InfoBox.DescriptionFrame.InfoBoxDescription.AutomaticSize = Enum.AutomaticSize.Y
...
setHoverTransition(nil, Color3.new(0.152941, 0.188235, 0.345098), ScriptBloxSearcher.MainFrame.InfoBox.BottomButtons.ExecuteButton)
setHoverTransition(nil, Color3.new(0.152941, 0.188235, 0.345098), ScriptBloxSearcher.MainFrame.InfoBox.BottomButtons.Close)
setHoverTransition(nil, Color3.new(0.152941, 0.188235, 0.345098), ScriptBloxSearcher.MainFrame.InfoBox.BottomButtons.CopyScriptBloxLink)
setHoverTransition(nil, Color3.new(0.152941, 0.188235, 0.345098), ScriptBloxSearcher.MainFrame.InfoBox.BottomButtons.CopyScriptButton)
...
function loadImage(url, id)	
	local path = "ScriptBlox/Searcher/ImageCache/"..id..".png"
...
function updateCache(newCache)
	cache = newCache
	writefile("ScriptBlox/Searcher_Cache.json", HttpService:JSONEncode(cache))
end
...
function updateInfoBox(scriptObj)
	...
	ScriptBloxSearcher.MainFrame.InfoBox.DescriptionFrame.InfoBoxDescription.Text = scriptObj.features or "For features to be displayed, please use the search bar to find this script and then open the info again, this is caused by an API Error that I will fix later."
	ScriptBloxSearcher.MainFrame.InfoBox.DescriptionFrame.CanvasPosition = Vector2.new(0, 0)
	
	ScriptBloxSearcher.MainFrame.InfoBox.Info.Author.ProfilePicture.Image = loadImage("https://scriptblox.com/"..scriptObj.owner.profilePicture, scriptObj["owner"]["_id"].."_p")
	ScriptBloxSearcher.MainFrame.InfoBox.Info.PreviewImage.Image = loadImage(_if(string.find(scriptObj.game.imageUrl, "rbxcdn.com"), scriptObj.game.imageUrl, "https://scriptblox.com/"..scriptObj.game.imageUrl), scriptObj["_id"].."_g")
	
	local ExecuteConnection
	local CopyLinkConnection
	local CopyScriptConnection
	
	ExecuteConnection = ScriptBloxSearcher.MainFrame.InfoBox.BottomButtons.ExecuteButton.MouseButton1Click:Connect(function()
		loadstring(scriptObj.script)()
	end)
	
	CopyLinkConnection = ScriptBloxSearcher.MainFrame.InfoBox.BottomButtons.CopyScriptBloxLink.MouseButton1Click:Connect(function()
		setclipboard("https://scriptblox.com/script/".. scriptObj.slug)
	end)
	
	CopyScriptConnection = ScriptBloxSearcher.MainFrame.InfoBox.BottomButtons.CopyScriptButton.MouseButton1Click:Connect(function()
		setclipboard(scriptObj.script)
	end)
	
	ScriptBloxSearcher.MainFrame.InfoBox.BottomButtons.Close.MouseButton1Click:Connect(function()
...
ScriptBloxSearcher.MainFrame.TopBarFrame.ClearCacheButton.MouseButton1Click:Connect(function()
	updateCache({})
	for _, v in pairs(listfiles("ScriptBlox/Searcher/ImageCache")) do
		delfile(_if(string.find(v, "ScriptBlox"), v, "ScriptBlox/Searcher/ImageCache/"..v))
	end
end)
